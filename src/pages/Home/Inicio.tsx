


const Inicio = () => {
    return (
        <>
            <div className="mt-24 flex w-10/12 flex-col items-center justify-center gap-12 self-center rounded-xl border-8 border-black p-10
            font-Nunito text-orange-600 shadow-[inset_0_3px_30px_8px_rgb(0,0,0)] md:p-16 lg:flex-row lg:p-12 xl:w-9/12">
                <div className="flex flex-col gap-4 text-center font-semibold text-shadow-md">
                    <h1 className="text-[2.75rem] md:text-7xl xl:text-7xl">Enzo Rocha</h1>
                    <h1 className="text-3xl md:text-5xl xl:text-5xl">Desenvolvedor FrontEnd</h1>
                </div>
                <img src={''} alt="foto" className="custom-border-radius size-64 border-4 border-orange-600 
            shadow-[0_0_15px_4px_rgb(0,0,0)]" />
            </div>
        </>
    )
}

export default Inicio